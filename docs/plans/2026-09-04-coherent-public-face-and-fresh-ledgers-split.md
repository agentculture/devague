# Implementation Split Plan — coherent public face and fresh ledgers

slug: `coherent-public-face-and-fresh-ledgers` · generated by `assign-to-workforce.sh split-plan --write` from `devague plan waves --json` (issue #82). Re-running this command overwrites this file in place; edits made to the **Task assignments** table below survive a regeneration, matched by task id.
...
- [unknown_nonblocking] Captured README output embeds devague 0.24.x ids and text; it will age with every release and nothing regenerates it. Mitigation is the version comment on each capture (h14); a refresh is a future chore, not this plan's
