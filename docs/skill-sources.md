...
`spec-to-plan`, `assign-to-workforce`, `deviate`, `validate-delivery`, and
`summarize-delivery` skills are **authored and maintained in this repo** —
devague is their origin/upstream, not a downstream consumer. The devague
agent dogfoods them to operate the devague CLI while improving the tool. The
flow runs the *opposite* direction of the table above: `guildmaster`
re-vendors them from here and re-broadcasts them to the mesh. (The skill
names are `scope` / `think` / `challenge` / `spec-to-plan` /
...
| `validate-delivery` | **devague** (here: `.claude/skills/validate-delivery/`) | `guildmaster`, then the AgentCulture mesh | Operator for the **execution-to-evidence** leg: runs after a plan's waves merge and before `/summarize-delivery` — run the confirmed plan's behavioral tests agent-side, then file record-only entries: `devague oblige` (mark a claim's behavioral obligation), `devague evidence` (obligation met by this test, asserting this behavior, outcome pass or fail), and `devague delta` (a behavioral added/amended/removed record, provenance back to the claim or approved deviation and forward to its evidence). The CLI never runs a test itself (#20); `--origin llm` lands proposed, same anti-fabrication contract as `deviate` and `lapse`. A failing or unchecked outcome is filed and reported as such — unmet is unmet. Behavioral tests are identified by a pytest marker (e.g. `@pytest.mark.behavioral`) or a dedicated folder (e.g. `tests/behavioral/`) — both defined in the skill. Motivating record: issues `agentculture/devague#97` and `agentculture/devague#107`. Method-only — no entry-point script; the `oblige`/`evidence`/`delta` CLI surface ships in a parallel task, so the skill is written against those verb names with minimal command examples for cheap reconciliation. New alongside the eight-leg renumbering. Re-vendor from `../devague/.claude/skills/validate-delivery/`. |
| `summarize-delivery` | **devague** (here: `.claude/skills/summarize-delivery/`) | `guildmaster`, then the AgentCulture mesh | Operator for the **delivery-side closure** leg (runs after `/assign-to-workforce` and `/validate-delivery`). Method-only in v1 — `SKILL.md` + template; no entry-point script, no new CLI verb. New in 0.17.0. Re-vendor from `../devague/.claude/skills/summarize-delivery/`. |
...
- **Don't re-vendor devague-origin skills.** `scope` / `think` / `challenge` /
  ...
  `summarize-delivery` are authored here; pull fixes forward into this repo,
  not back from guildmaster's re-broadcast copies.
