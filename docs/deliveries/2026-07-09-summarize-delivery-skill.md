...
| The devague CLI is unchanged by this release | high | commits `a5ea5e0`..`39d102a` touch no file under `devague/` |
| The full suite passes and tracked markdown lints clean | high | `uv run pytest -n auto` → 418 passed · markdownlint → 23 files, 0 errors |
...
| `colleague work` returns exit `0` on silent failure | high | 4 of 5 attempts: `status: incomplete`, `steps: 0`, `changed files: (none)`, exit `0` — flight ids above |
| The final PR gate (human gate 3) has passed | unverified | the PR is not yet opened at the time of writing — not claimed |
